...
for i, elements_per_wavelength in enumerate(elements_per_wavelength_ls):
    for j, model_order in enumerate(model_order_ls):
        ...
        # add random layers
        for k,l in enumerate(l_ls):
            layer_ls.append(sn.layered_meshing.interface.Hyperplane.at(interface_ls[k]))
            layer_ls.append(sn.material.elastic.triclinic.TensorComponents.from_params(**matl.rotated_parameters(theta_ls[k])))
