...
p = sn.Project.from_domain(path=Path(PROJECT_DIR_WIN, 'rotated_triclinic'), 
...
rxs_pos = [Vector(x_length/2, y_range[1]/2, z_range[1]/2)]

rxs_middle = [Point3D(x=r.x, y=r.y, z = r.z, 
            station_code=f"REC_middle",
            # Note that one can specify the desired recording field here.
            fields=fileds,)
        for i, r in enumerate(rxs_pos)
        ]
...
# rotate 45 degree.
material_unoriented = sn.material.elastic.triclinic.TensorComponents.from_params(**matl.rotated_parameters(np.pi/3))

# material_unoriented = sn.material.elastic.hexagonal.TensorComponents.from_params(
#     rho = matl.RHO,
#     c11 = matl.C11,
#     c12 = matl.C12,
#     c13 = matl.C13,
...
layered_model = sn.layered_meshing.LayeredModel([material_unoriented])
